...
##################################################################################################
# original model fit
lm_fit <- readRDS(file.path("..", "Saha 2021 model", "ufp_model", "data", "modified", "lm_fit.rda"))

modeling_cov <- names(lm_fit$coefficients) %>% setdiff("(Intercept)")

##################################################################################################
# model fit data
modeling_data <- read.csv(file.path("..", "Saha 2021 model", "ufp_model", "data", "modified", "input_used.csv")) %>%
...
compare %>%
  ...
  scale_x_log10() +
  labs( 
       subtitle = "x-axis is on the log10 scale (drops values<=0)"
       )


# histogram of m_to_truck
...
```{r}
compare %>%
  filter(data_type=="modeling") %>%
  # put on the modeling scale
  mutate(#value = ifelse(name=="m_to_truck", 1/value, value)
         value = ifelse(grepl("m_to_truck", name), log(value), value),
         name = ifelse(grepl("m_to_truck", name), paste0("log_", name), value),
         ) %>%
  
  ggplot(aes(x=value, y=pnc)) + 
  facet_wrap(~name, scales="free", switch="x") + 
  geom_point(alpha=0.3) + 
  geom_smooth() #+ geom_smooth(method="lm", se=F,col="black") 
