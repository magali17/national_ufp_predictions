...
* to address extreme UFP predictions (e.g., Manhattan), we winsorize the predictions below/above the specific quantiles
...
ufp_2010 %>%
  ...
  labs(title = "Map of blocks with missing covariates (i.e., predictions)")
...
# Distribution of UFP Predictions after winsorization

```{r}
bind_rows(ufp_2010#, 
          #ufp_2020 
          ) %>%
  group_by(block_year=year) %>%
  summarize(
    n = n(),
    missing = sum(is.na(ufp_winsorized)),
    min=min(ufp_winsorized, na.rm = T),
    Q01 = quantile(ufp_winsorized, probs = 0.01, na.rm = T),
    Q05 = quantile(ufp_winsorized, probs = 0.05, na.rm = T),
    Q25 = quantile(ufp_winsorized, probs = 0.05, na.rm = T),
    Q50 = quantile(ufp_winsorized, probs = 0.50, na.rm = T),
    Q75 = quantile(ufp_winsorized, probs = 0.75, na.rm = T),
    Q95 = quantile(ufp_winsorized, probs = 0.95, na.rm = T),
    Q99 = quantile(ufp_winsorized, probs = 0.99, na.rm = T),
    max = max(ufp_winsorized, na.rm = T)
  )
...
```{r}
ufp_2010 %>%
  filter(winsorized==TRUE) %>%
  st_as_sf(coords = c('long_block', 'lat_block'), crs=project_crs) %>%
  
  ggplot() + 
  geom_sf(data = states, fill=alpha(0.01)) +
  geom_sf_text(data = states, aes(label=STUSPS)) +
  geom_sf(aes(col=winsorized), alpha=0.4, size=0.5) + 
  labs(title = "Map of blocks with winsorized UFP predictions")
...
ufp_2010 %>%
  ...
  summarize(
    no_blocks=n(),
    proportion_winsorized = mean(winsorized, na.rm = T)) %>%
  arrange(-proportion_winsorized) %>%
  kable(caption = "Proportion of blocks with winsorized UFP predictions") %>%
  kable_styling()
